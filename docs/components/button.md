...
| `size` |  | 静态值 |  |  |
| `block` |  | 静态值 |  |  |
| `debounce` |  | 静态值 | 首次点击立即执行，防抖时间内的重复点击会被忽略并重置倒计时；配置为 0 时关闭。 | `300` |
| `children` |  | 静态值 |  |  |
| `on_tap` |  | 静态值 |  |  |
| `name` |  | 静态值 |  |  |
| `domId` |  | 静态值 |  |  |
...
控制按钮点击后的重复触发间隔，单位为毫秒。首次点击会立即执行；在间隔时间内再次点击会被忽略并重置倒计时。只有停止点击满 `debounce` 毫秒后，下一次点击才会执行；倒计时结束不会自动补执行。未配置时默认为 `300`，配置为 `0` 可以关闭防抖。该配置仅适用于普通 `button` 组件，不影响 `float_button`。

例如，默认配置下在 0ms、100ms、250ms 连续点击，仅 0ms 执行一次；550ms 恢复可执行状态。
