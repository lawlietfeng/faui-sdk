...
export const InputNumber: React.FC<ComponentProps<'inputnumber'>> = ({ config }) => {
  ...
  const initialValue = path ? (dataValue as number | string | null) : null;
  ...
  useEffect(() => {
    if (path) {
      const newValue = dataValue as number | string | null;
      setValue(newValue);
    }
  }, [path, dataValue]);
