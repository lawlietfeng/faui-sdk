...
export const Select: React.FC<ComponentProps<'select'>> = ({ config }) => {
  ...
  const initialValue = normalizeSelectValue(path ? dataValue : undefined);
  const [value, setValue] = useState(initialValue as string | undefined);
  const fieldName = resolveFieldName(config);
  const rules = useMemo(() => normalizeRules(config.rules), [config.rules]);

  useEffect(() => {
    if (path) {
      const newValue = normalizeSelectValue(dataValue);
  ...
  const handleChange = useCallback((newValue: string) => {
    const normalizedValue = normalizeSelectValue(newValue) as string | undefined;
    setValue(normalizedValue);
    formContext?.syncFieldValue(config.id, normalizedValue ?? '');
    void formContext?.validateField(config.id);

    if (config.on_change) {
      const [resolved, extra] = resolveOnChange(config.on_change, normalizedValue ?? '');
  ...
  return (
    <div>
      <AntdSelect
        ...
        allowClear={evaluatedAllowClear}
        showSearch={evaluatedShowSearch}
        maxTagCount={evaluatedMaxTagCount}
