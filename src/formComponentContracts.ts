...
const contracts: FormComponentContract[] = [
  ...
  contract('button', { props: ['label', 'title', 'color', 'content', 'disabled', 'type', 'danger', 'ghost', 'shape', 'size', 'block', 'debounce', 'children', 'on_tap'], childrenMode: 'component-ids', properties: { label: labelProperty, content: contentProperty, disabled: disabledProperty, debounce: { schema: schema('number'), default: 300, description: '首次点击立即执行，防抖时间内的重复点击会被忽略并重置倒计时；配置为 0 时关闭。' }, on_tap: { schema: schema(['object', 'array']) } }, events: { on_tap: { description: '按钮点击时执行。' } } }),
